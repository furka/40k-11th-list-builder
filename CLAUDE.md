...
- **Source files**: `src/data/munitorum-field-manual-11th/` holds `current/` (the live/latest version) and `historical/v<siteVersion>-<scrapedAt>/` (archived prior versions). Each snapshot is a **complete, standalone dense set** — a `_manifest.json`, a `_changes.md` (human-readable diff vs the prior version), and one JSON per faction. There is no sparse overlay to reconstruct: each dir stands alone. `current/` is rewritten **in place** on each scrape so PRs show clean per-faction point diffs; the prior `current/` is archived into `historical/` before it is overwritten (see `scripts/scrape-mfm-11th/index.mjs`). Read snapshots via `snapshot-resolve.mjs` (Node) or `index.js` (Vite/browser).
- **Parser**: `src/utils/data-reader-11th.js` — `parse11thFaction()` and `parse11thSnapshot()` produce `{ FACTIONS, DATA_SHEETS, MFM_VERSION }` from a snapshot.
- **Aggregator**: `src/data/munitorum-field-manual-11th/index.js` — `load11thMFM()` auto-globs `current/` + `historical/*/` at build time, parses each standalone, and produces `{ "V1.0": {...}, ..., CURRENT, PREVIOUS }` (`current/` is CURRENT, newest historical is PREVIOUS). Adding a new version is zero-config.
...
There are **two independent scrapers**:

- **`scripts/scrape-mfm-11th/`** (`npm run mfm:scrape`) — scrapes the GW MFM **website** for points and writes `current/`. Points-only; needs no API key. If content is unchanged it's a no-op; if it changed it archives the old `current/` to `historical/` and rewrites `current/` in place (plus a fresh `_changes.md`); the aggregator picks it up automatically.
- **`scripts/scrape-faction-pack-11th/`** (`npm run factionpack:scrape`) — classifies the GW Faction Pack **PDFs** into the `.auto.json` layers. Reads its datasheet/detachment/enhancement vocabulary from the committed snapshot (via `snapshot-resolve.mjs`), so it runs independently of the MFM scrape (its CI job runs ~30 min after the MFM one). Needs `ANTHROPIC_API_KEY`. Every PDF page is first typed (`page-types.mjs`: datasheet / detachment / rulesUpdate / other) and each pass only reads its relevant page types. It runs four passes: datasheet keywords (`faction-pack-keywords.auto.json`), enhancement restrictions (`enhancement-restrictions.auto.json`), detachment BATTLELINE grants (`conditional-battleline.auto.json`), and Rules-Updates keyword errata (`errata-keywords.auto.json`, applied as an add/remove delta in `src/data/keywords/index.js`).

**Other snapshot consumers** (`scripts/audit-and-prune-overrides.mjs`, `scripts/scrape-bsdata-{enhancements,wargear}/index.mjs`, `scripts/scrape-faction-pack-11th/audit-restrictions.mjs`) all go through `snapshot-resolve.mjs`'s `resolveSnapshotState`/`resolveSnapshotStateSync`, which returns the full `current/` (latest) state by default (pass `{ upTo }` to read a specific historical version).
