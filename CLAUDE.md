...
- **`scripts/scrape-mfm-11th/`** (`npm run mfm:scrape`) — scrapes the GW MFM **website** for points and writes the snapshot dirs. Points-only; needs no API key. Either updates files in place (content unchanged, no new dir) or mints a new sparse snapshot dir + `_changes.md`; the aggregator picks it up automatically.
